...
public class Kunai : ChessPieceBase
{
	...
	public override bool Attack(Transform target)
	{
		...
		if (transform.localPosition.z + 1 == target.localPosition.z && target.localPosition.x == transform.localPosition.x + 2
		|| transform.localPosition.z + 1 == target.localPosition.z && target.localPosition.x == transform.localPosition.x - 2
		|| transform.localPosition.z - 1 == target.localPosition.z && target.localPosition.x == transform.localPosition.x + 2
		|| transform.localPosition.z - 1 == target.localPosition.z && target.localPosition.x == transform.localPosition.x - 2)
		{
			transform.localPosition = new Vector3(target.localPosition.x, transform.localPosition.y, target.localPosition.z);
	...
	public override void HighlightMoves()
	{
		...
		if (MyColor.ToString().CompareTo("Black") == 0)
		{
			...
			{
				if ((transform.localPosition.x + 2 == space.transform.localPosition.x && transform.localPosition.z == space.transform.localPosition.z)
				|| (transform.localPosition.x - 2 == space.transform.localPosition.x && transform.localPosition.z == space.transform.localPosition.z))
				{
					...
					foreach (GameObject enemy in enemies)
					{
						if (transform.localPosition.z + 1 == enemy.transform.localPosition.z && enemy.transform.localPosition.x == transform.localPosition.x + 2
						|| transform.localPosition.z + 1 == enemy.transform.localPosition.z && enemy.transform.localPosition.x == transform.localPosition.x - 2
						|| transform.localPosition.z - 1 == enemy.transform.localPosition.z && enemy.transform.localPosition.x == transform.localPosition.x + 2
						|| transform.localPosition.z - 1 == enemy.transform.localPosition.z && enemy.transform.localPosition.x == transform.localPosition.x - 2)
						{
