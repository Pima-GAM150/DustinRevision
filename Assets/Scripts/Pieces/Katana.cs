...
public class Katana : ChessPieceBase
{
	#region Variables
	...
	public override bool Attack(Transform target)
	{
		...
		if ((transform.localPosition.x == target.localPosition.x && transform.localPosition.z + 2 == target.localPosition.z)
		...
		if (MyColor.ToString().CompareTo("Black") == 0)
		{
			foreach (GameObject space in Board.Instance.Spaces)
			{
				...
				|| (transform.localPosition.x - 1 == space.transform.localPosition.x && transform.localPosition.z - 1 == space.transform.localPosition.z))
				{
					...
					foreach (GameObject enemy in enemies)
					{
						if ((transform.localPosition.x == enemy.transform.localPosition.x && transform.localPosition.z + 2 == enemy.transform.localPosition.z)
						|| (transform.localPosition.x == enemy.transform.localPosition.x && transform.localPosition.z - 2 == enemy.transform.localPosition.z))
						{
